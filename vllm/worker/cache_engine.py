...
from vllm.model_executor.parallel_utils.parallel_state import (
    get_stage_model_parallel_next_rank,
    get_stage_model_parallel_prev_rank)
...
class CacheEngine:
    ...
    def send_blocks(self, block_ids: List[int]) -> None:
        ...
        reqs.append(
            torch.distributed.isend(
                value_tensor, dst=get_stage_model_parallel_next_rank()))
        return reqs
    ...
    def recv_blocks(self, block_ids: List[int]) -> None:
        ...
        reqs.append(
            torch.distributed.irecv(
                key_tensor, src=get_stage_model_parallel_prev_rank()))
        reqs.append(
            torch.distributed.irecv(
                value_tensor, src=get_stage_model_parallel_prev_rank()))
        ...
        for block_id in block_ids:
            for i in range(self.num_layers):
                self.gpu_cache[i][0][block_id].copy_(
                    key_tensor[offset])
                self.gpu_cache[i][1][block_id].copy_(
                    value_tensor[offset])
                offset += 1
